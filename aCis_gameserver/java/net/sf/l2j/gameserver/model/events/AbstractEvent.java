...
public abstract class AbstractEvent implements IEvent {
    ...
    public boolean areRequiredPlayersRegistered() {
        var arePlayersEnough = !registeredPlayers.isEmpty() && registeredPlayers.size() >= 2;
        if (!arePlayersEnough)
            announceNpc(EventConstants.INSUFFICIENT);
        return arePlayersEnough;
    }
    ...
    public static void announceNpc(String msg) {
        if (npc != null) {
            final RestartPoint creatureRp = RestartPointData.getInstance().getRestartPoint(npc);
            final CreatureSay cs = new CreatureSay(npc, SayType.SHOUT, msg);

            World.getInstance().getPlayers().stream().filter(Player::isOnline).forEach(p -> {
                final RestartPoint plRp = RestartPointData.getInstance().getRestartPoint(p);
                if (creatureRp.equals(plRp))
                    p.sendPacket(cs);
            });
        } else {
            World.getInstance().getPlayers().stream().filter(Player::isOnline).forEach(p -> p.sendMessage(msg));
        }
    }
